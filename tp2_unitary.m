...
function dxdt = combined_ode(t, x, Kp, Kd, a, l, m, r, btp, A, B, C, D)
    ...
    [g_est,dphi] = approx_grav(a,m,l,r, btp, theta, omega, e, phi);

    % Unitary control
    lambda_b = 5;
    
    if abs(btp*e) > 0 
        lambda = -btp*e*lambda_b*sin(theta)/norm(btp*e,2);
    else
        lambda = 0;
    end
    
    den = (4*l/3) - a * m * l * cos(theta)^2;
    u = (g_est * sin(theta) + lambda - ((a * m * l * omega^2 * sin(2*theta)) / 2) - den * (-Kp * e_theta - Kd * e_omega + ddot_theta_d)) / (a * cos(theta));
...
function [g_est,dphi] = approx_grav(a,m,l, r, btp, theta, omega, e, phi)

    g_est = 0;
    dphi = zeros(r,1); 
    Gamma = 0.01; %adaptive control law
