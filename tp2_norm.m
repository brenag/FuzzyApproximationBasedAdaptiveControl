...
function [g_est,dphi] = approx_grav(a,m,l, r, btp, theta, omega, e, phi)
    ...
    nu = 0.1; %norm estimation
    gamma = 1000; %adaptive control law
    delta = 0.01;

    zeta = pertinencias(theta,r);  
    s = zeta'*zeta;

    g_est = (phi*s/(2*nu))*(btp*e)/(4*l/3 - a*m*l*(cos(theta)^2));
    
    if norm(btp*e,2) > delta     
        dphi = (gamma/(2*nu))*s*(btp*e)'*(btp*e)/(4*l/3 - a*m*l*(cos(theta)^2))^2;
    else
        dphi = 0;
    end
end
